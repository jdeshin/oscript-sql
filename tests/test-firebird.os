...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	СуфикМетодов = "";
	мСтрокаСоединения = ПолучитьТекстИзФайла("fixtures\firebird-con-str.txt");
...
Процедура Тест_Должен_ПроверитьГенерациюСтрокиСоединения() Экспорт
	...
	Соединение.ТипСУБД = Соединение.ТипыСУБД.Firebird;
	Соединение.Сервер = "localhost";
	Соединение.ИмяПользователя = "SYSDBA";
	Соединение.ИмяБазы = "SampleDatabase.fdb";
	Соединение.Пароль = "masterkey";
	Соединение.Порт = 3050;
	...
	Попытка
		...
		Соединение.Открыть();
	Исключение
		Ожидаем.Что(Соединение.СтрокаСоединения).Равно("DataSource=localhost;User=SYSDBA;Password=masterkey;Database=SampleDatabase.fdb;Port=3050;");
	КонецПопытки;
...
Процедура Тест_Должен_СоздатьТаблицу() Экспорт
	...
	// Исключение
	// 	// Сообщить("state:: " + Соединение.Открыто); 
	// 	// Ожидаем.Что(Соединение.СтрокаСоединения).Равно("DataSource=localhost;User=SYSDBA;Password=masterkey;Database=SampleDatabase.fdb;Port=3050;");
	// КонецПопытки;
